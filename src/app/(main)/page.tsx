...
export default function DashboardPage() {
  ...
  const handleUserVote = async (report: IncidentReport, voteType: 'confirm' | 'dispute') => {
    ...
    try {
        ...
        if (voteType === 'confirm' && newVotesCount >= VOTE_THRESHOLD) {
            newStatus = 'confirmed';
            reputationChange = 1;
            notificationType = 'report_confirmed';
        } else if (voteType === 'dispute' && newVotesCount >= VOTE_THRESHOLD) {
            newStatus = 'disputed';
            reputationChange = -1;
  ...
  const renderReportActions = (report: IncidentReport) => {
              ...
              <>
                ...
                <div className="grid gap-4 md:hidden">
                  {highPriorityIncidents?.map((report) => (
                    <div key={report.id} className="border-b px-4 pb-4 last:border-b-0">
                      ...
                      <p className="text-sm text-muted-foreground py-2">{report.summary || report.description}</p>
